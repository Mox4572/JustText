...
//  JustText2
//
//  Created by MDNich on 2/25/22.
//
// COPY PLEASE DO NOT DESTROY

import Cocoa

class DocumentOld: NSDocument {
   ...
   override func read(from fileWrapper: FileWrapper, ofType typeName: String) throws {
       ...
       Swift.print(typeName)
        isRTF = true
        if let viewCtrl = NSApplication.shared.mainWindow?.windowController?.contentViewController as? ViewController, let textView = viewCtrl.textView {
            // TODO
            guard let attrText = NSAttributedString(rtfdFileWrapper: fileWrapper, documentAttributes: nil) else { Swift.print("sorry buddy")
                return }
    ...
    override func read(from data: Data, ofType typeName: String) throws {
        ...
        if(typeName == "com.apple.rtfd") {
            isRTF = true
            if let contents = NSAttributedString(rtfd: data, documentAttributes: nil) {
                text = contents
            }
        }
    }
